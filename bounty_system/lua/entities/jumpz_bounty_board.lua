...
function ENT:Initialize()
    if SERVER then
        self:SetModel("models/ace/sw/rh/cgi_console_01.mdl")
...
function ENT:Use(activator)
    ...
    net.Start("Jumpz_OpenBountyBoard")
...
function ENT:Draw()
    ...
    cam.Start3D2D(self:LocalToWorld(Vector(0,0,self:OBBMaxs().z)) + Vector(0,0,9), ang, 0.075)
        draw.SimpleText("Bounty Board", "NCS_DATAPAD_OVERHEAD", 0, 25, color_white, TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER)
    cam.End3D2D()
end
